---
layout: archive
title: "Curriculum Vitae"
permalink: /cv_old/
author_profile: true
---

# Curriculum Vitae

My curriculum vitae summarizes my education, research, teaching, publications, honors, and professional experience. A complete and up-to-date version of my CV can be downloaded below.

📄 **[Download Full Curriculum Vitae (PDF)](/files/Md_Zakir_Hossain_CV.pdf)**

---

## Education

### University of Connecticut
**Ph.D. Candidate, Computer Science and Engineering**  
**Expected Graduation:** May 2027

**Research Areas**

- Artificial Intelligence for Healthcare
- Satellite-Based Quantum Key Distribution

**Advisor**

- Prof. Bing Wang

---

### Dhaka University of Engineering and Technology (DUET)

**M.Sc. in Computer Science and Engineering**

---

### Dhaka University of Engineering and Technology (DUET)

**B.Sc. in Computer Science and Engineering**

Awarded the **University Gold Medal** and **Dean's Award** for outstanding academic performance.

---

## Academic Experience

### University of Connecticut

**Research Assistant** *(2023 – Present)*

- Conduct research in Artificial Intelligence for Healthcare and Satellite-Based Quantum Key Distribution.
- Develop machine learning models for predicting depression treatment outcomes using longitudinal behavioral data.
- Design routing and scheduling algorithms for satellite-based quantum communication networks.
- Publish research papers in leading journals and conferences.

---

### University of Connecticut

**Graduate Teaching Assistant** *(Approximately 2 Years)*

- Supported undergraduate computer science and mathematics courses.
- Conducted laboratory and discussion sessions.
- Held office hours and assisted students with programming assignments.
- Graded assignments, quizzes, and examinations.
- Evaluated answer scripts and maintained student grade records.

---

### Dhaka University of Engineering and Technology (DUET)

**Assistant Professor** *(August 2019 – December 2022)*

**Lecturer** *(July 2017 – August 2019)*

- Taught undergraduate computer science courses.
- Developed lectures, laboratory exercises, assignments, and examinations.
- Supervised undergraduate theses and capstone projects.
- Mentored students in programming, software development, and machine learning.

---

### Bangladesh University

**Lecturer** *(January 2016 – July 2017)*

- Taught undergraduate computer science courses.
- Delivered courses in artificial intelligence, databases, computer networks, and object-oriented programming.
- Guided students through programming assignments and practical laboratory activities.

---

### IECL, Dhaka, Bangladesh

**Software Engineer** *(June 2015 – December 2015)*

- Developed Android application features using Android Studio.
- Built back-end components using PHP and CakePHP.
- Contributed to a government project management system using HTML, CSS, JavaScript, and MySQL.
- Collaborated with software developers to design, implement, and maintain web-based applications.

---

---

## Technical Skills

**Programming Languages:**  
Python, R, MATLAB, Java, C, C++, SQL

**Machine Learning & AI:**  
PyTorch, TensorFlow, LSTM, GRU, GRU-D, XGBoost, Random Forest, Support Vector Machines (SVM), Explainable AI (SHAP), Contrastive Learning, Time-Series Analysis

**Large Language Models & Transformers:**  
Transformer Encoder, PatchTST, Self-Supervised Learning, Positional Encoding, Sequence Representation Learning

**Graph & Network Learning:**  
Graph Neural Networks (GCN), Reinforcement Learning, Continual Learning, Satellite Network Optimization

**Web & Software Development:**  
HTML, CSS, JavaScript, Bootstrap, React.js, PHP, CakePHP, Android Development

**Databases:**  
MySQL, PostgreSQL, SQL Server, Oracle, SQLite, NoSQL

**Cloud & Development Tools:**  
Git, GitHub, Docker, AWS, Microsoft Azure

---

For a complete record of my education, research, teaching experience, publications, honors, professional service, and technical skills, please download my full Curriculum Vitae.

📄 **[Download Full Curriculum Vitae (PDF)](/files/Md_Zakir_Hossain_CV.pdf)**
